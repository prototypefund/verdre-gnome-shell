...
var DndGesture = GObject.registerClass({
    ...
    _gestureRecognizing() {
        ...
        global.stage.connect('captured-event', (actor, event) => {
            if (event.type() === Clutter.EventType.KEY_PRESS && this.state === Clutter.GestureState.RECOGNIZING) {
...
log("DRAG: someone called startDrag");
    ...
    vfunc_state_changed(oldState, newState) {
        ...
        if (oldState === Clutter.GestureState.RECOGNIZING &&
            ...
            const latestEventTime = this.get_points()[0].event_time;

            this.emit('drag-cancelled', latestEventTime);

            if (this._actorDestroyed || !this._dragActor) {
                global.display.set_cursor(Meta.Cursor.DEFAULT);
                this._dragComplete();
                this.emit('drag-end', latestEventTime, false);
            ...
            this._animateDragEnd(latestEventTime, {
                x: snapBackX,
                y: snapBackY,
