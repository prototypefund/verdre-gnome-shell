...
g_warning("CURRENT STING %s", data->key);
  g_hash_table_remove (priv->outstanding_requests, data->key);

  if (pixbuf == NULL)
    {
g_warning("TEXTURE_CACHE: pixbuf is NULL, returning errors");
  ...
  if (data->policy != ST_TEXTURE_CACHE_POLICY_NONE)
    {
      /* FIXME: Is it possible that someone puts this into the cache before we finish loading?? */
...
g_warning("TEXTURE_CACHE: imag null");
          for (iter = data->tasks; iter; iter = iter->next)
            g_task_return_error (G_TASK (iter->data), error);
                                   ...
                                   gpointer             user_data)
{
  ...
  request_texture_from_cache (cache, NULL, key, policy,
